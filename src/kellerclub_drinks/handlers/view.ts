...
class SingleQuery<T extends Element> implements ElementQuery<Element> {
    ...
    withId(id: string) {
        ...
        throw err(this, `${nodeToStr(this.#node)} does not have ID ${id}!`);
    }
    ...
    withClass(className: string) {
        ...
        else throw err(this, `${nodeToStr(this.#node)} does not have class ${className}!`);
    }
    ...
    withTag<T extends keyof HTMLElementTagNameMap>(tag: T): SingleQuery<HTMLElementTagNameMap[T]> {
        ...
        else throw err(this, `${nodeToStr(this.#node)} does not have tag ${tag}!`);
    }
}
...
class MultiQuery<T extends Element> implements ElementQuery<Element[]> {
    ...
    withId(id: string) {
        ...
        else throw err(this, `${!result.length ? 'No node' : 'More than one node'} has ID ${id} in ${this.#nodes.map(node => nodeToStr(node)).join(',')}!`);
    }
    ...
    withName(name: string) {
        ...
        else throw err(this, `${!result.length ? 'No node' : 'More than one node'} has name ${name} in ${this.#nodes.map(node => nodeToStr(node)).join(',')}!`);
    }
    ...
    withClass(className: string) {
        ...
        else throw err(this, `${!result.length ? 'No node' : 'More than one node'} has class ${className} on ${this.#nodes.map(node => nodeToStr(node)).join(',')}!`);
    }
    ...
    withTag<T extends keyof HTMLElementTagNameMap>(tag: T): SingleQuery<HTMLElementTagNameMap[T]> {
        ...
        else throw err(this, `${!result.length ? 'No node' : 'More than one node'} has tag ${tag} on ${this.#nodes.map(node => nodeToStr(node)).join(',')}!`);
    }
}
